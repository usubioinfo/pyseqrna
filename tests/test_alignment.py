...
def mock_star_config():
    return {
        "alignment": {"--outSAMtype": "BAM SortedByCoordinate"},  # STAR uses 'alignment' section for run_alignment params
        "star": {"--runThreadN": "4"},  # STAR uses 'star' section for index building
    }
...
def test_run_alignment(
    mock_execute, mock_check_index, mock_verify, mock_load_config, mock_which, mock_star_config, temp_outdir
):
...
def test_star_read_files_command_uses_resolved_executable(mock_verify, mock_load_config, mock_which, temp_outdir):
    ...
    mock_load_config.return_value = {"alignment": {"zipped_file": "--readFilesCommand zcat"}}
    mock_verify.return_value = True
