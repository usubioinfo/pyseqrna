...
"""
Base Read Trimmer Module

This module defines the abstract base class and custom exceptions for read trimmer implementations
in pySeqRNA. The ReadTrimmer class provides a common interface, configuration loading, CPU thread
optimization, and job execution management (both local and SLURM) for different trimming tools.

Features:
    - Abstract base class representing a unified API for trimming tools
    - Automatic CPU thread resolution and resource management
    - Standardized job execution (local multi-processing or SLURM scheduler integration)
    - Automated sample and read file validation for raw FASTQ inputs
    - Output directory management and output validation for single-end and paired-end runs
    - Configurable execution dry-runs simulating command preparation
...
Configuration:
    ...
    for individual tools are loaded from external configuration files (e.g. trim_galore.ini) via ConfigManager.
...
Classes / Functions / Exceptions:
    - TrimmingError: Custom exception for trimming-related errors.
    - ReadTrimmer: Abstract base class outlining standard trimming, execution, and validation interfaces.
...
class TrimmingError(Exception):
    """Custom exception for trimming-related errors."""

    pass


class ReadTrimmer(ABC):
    """
    Base abstract class for read trimmers.

    This class defines the common interface and provides shared functionality
    for all read trimmer implementations. Specific trimmer implementations
    should inherit from this class and implement the required abstract methods.

    Attributes:
        name (str): Name of the trimmer
        ...
        logger (logging.Logger): Logger for the trimmer
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        Initialize the ReadTrimmer base class.
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments for specific trimmer implementations

        Raises:
            ValueError: If sample_dict is invalid or empty
            FileNotFoundError: If param_dir is specified but doesn't exist
        """
        # Initialize logger
        if logger is None:
        ...
        # Extract class name and set trimmer name unless a subclass already
        # provided the canonical tool name (for example, "trim_galore").
        if not getattr(self, "name", None):
            self.name = self.__class__.__name__.replace("Trimmer", "").lower()
        self.logger.debug(f"Initializing {self.name} trimmer")
        ...
        # Perform additional initialization specific to the trimmer type
        ...
        self.logger.debug(f"{self.name} trimmer initialization complete")
    ...
    def _get_threads_from_config(self) -> Optional[int]:
        ...
        Different trimmer tools use different parameter names for threads, so this
        method checks several common patterns.

        Returns:
            Thread count if found in config, None otherwise
        """
        # Determine config files to try based on trimmer and paired setting
        config_files_to_try = []

        # Add tool-specific config files
        if self.name == "trimmomatic":
            config_files_to_try.append(f"trimmomatic{'PE' if self.paired else 'SE'}.ini")
        else:
            config_files_to_try.append(f"{self.name}.ini")
            ...
            try:
                ...
                for thread_param in [
                    ...
                    if thread_param in tool_section:
                        ...
                        # Handle different formats like "-threads 8" or just "8"
                        if isinstance(thread_value, str):
                            # Extract number from string like "-threads 8" or "--threads=8"
                            import re
    ...
    def _validate_sample_dict(self, sample_dict: Dict[str, List[str]]) -> None:
        ...
        for sample, reads in sample_dict.items():
            ...
            for read_file in file_paths:
                ...
                if not self.file_manager.verify_files_exist(read_file):
    ...
    def _init_specific(self, **kwargs: Any) -> None:
        """
        Initialize attributes specific to the trimmer implementation.
    ...
    def load_config(self, default_config_name: str) -> Dict[str, Any]:
        """
        Load the tool-specific configuration file, falling back to default if custom config fails.

        Args:
            default_config_name: Name of the default config file

        Returns:
            Dict containing configuration parameters
        """
        # Use ConfigManager's dedicated tool config method instead of direct implementation
        config_manager = ConfigManager(logger=self.logger)

        # Try user-specified param directory first if provided
        if self.param_dir:
    ...
    def prepare_command(self, sample_name: str, reads: List[str]) -> str:
        """
        Prepare the command string for trimming the specified sample.

        This method must be implemented by each trimmer to construct the
        command string for the specific trimming tool.
    ...
    def prepare_output_file(self, sample_name: str) -> Union[str, Tuple[str, str]]:
        """
        Prepare output file path(s) for the specified sample.

        For single-end reads, this returns a single output file path.
        For paired-end reads, this returns a tuple of output file paths (R1, R2).

        Args:
            sample_name: Name of the sample

        Returns:
            Output file path(s) for the sample
        """
        # Use the quality and trimming directory created by pipeline
        quality_trim_dir = Path(self.out_dir) / "1.Quality_and_trimming"
        out_dir = quality_trim_dir / f"{self.name}_results"
        ...
        if self.paired:
            r1 = str(out_dir / f"{sample_name}_R1_trimmed.fastq.gz")
            r2 = str(out_dir / f"{sample_name}_R2_trimmed.fastq.gz")
            return (r1, r2)
        else:
            return str(out_dir / f"{sample_name}_trimmed.fastq.gz")
    ...
    def validate_output_files(self, output_files: Union[str, Tuple[str, str]]) -> bool:
        ...
        Args:
            output_files: Path or tuple of paths to output files
        ...
        if isinstance(output_files, tuple):
            ...
            if not r1_exists:
                ...
                self.logger.debug(f"File exists check: {os.path.exists(output_files[0])}")

            if not r2_exists:
                self.logger.debug(f"Output file R2 missing: {output_files[1]}")
                self.logger.debug(f"File exists check: {os.path.exists(output_files[1])}")
            ...
            if not exists:
                ...
                self.logger.debug(f"File exists check: {os.path.exists(output_files)}")
            return exists
    ...
    def execute_command(self, commands: Dict[str, str], output_dir: str, tool_name: str) -> Dict[str, str]:
        ...
        Raises:
            TrimmingError: If command execution fails
        """
        try:
            ...
            raise TrimmingError(f"Command execution failed: {str(e)}")

    def run(self) -> Dict[str, Union[str, Tuple[str, str]]]:
        """
        Run trimming for all samples in the sample dictionary.
        ...
        Returns:
            Dictionary mapping sample names to output file(s)

        Raises:
            TrimmingError: If command execution fails for any sample
        """
        self.logger.info(f"Running {self.name} trimmer for {len(self.sample_dict)} samples")
        ...
        for sample_name, sample_info in self.sample_dict.items():
            ...
            results[sample_name] = output_files
            self.logger.debug(f"Output files for {sample_name}: {output_files}")
            ...
            try:
                ...
                raise TrimmingError(f"Command preparation failed for {sample_name}: {str(e)}")
        ...
        # If not using SLURM, validate output files after local execution.
        if not self.slurm:
            self.logger.debug("Validating output files")
            for sample_name, output_files in results.items():
                self.logger.debug(f"Checking output files for {sample_name}")
                if not self.validate_output_files(output_files):
                    self.logger.warning(f"Output files missing for {sample_name}")

        # Store job IDs when commands were submitted through SLURM. These IDs
        # are needed so later stages can submit dependent jobs.
        ...
        self.logger.info(f"{self.name} trimming completed successfully for all samples")
        return results
    ...
    def check_dependencies(self) -> bool:
        ...
        """
        trimmer_cmd = self._get_trimmer_command()

        try:
            # Check if command is in PATH
            if not shutil.which(trimmer_cmd):
                if self.dryrun:
                    self.logger.warning(f"{self.name} command not found in PATH: {trimmer_cmd}; continuing in dry-run mode")
                    return True
                self.logger.error(f"{self.name} command not found in PATH: {trimmer_cmd}")
                return False
    ...
    def _get_trimmer_command(self) -> str:
        """
        Get the base command for the trimmer tool.

        This method should be overridden by subclasses if the command name
        differs from the trimmer name.

        Returns:
            Base command for the trimmer
        """
        return self.name
    ...
    def get_version(self) -> str:
        """
        Get the version of the trimming tool.

        Returns:
            Version string of the trimming tool
    ...
    def validate_arguments(self, args: List[str], valid_args_list: List[str]) -> List[str]:
        ...
        Raises:
            TrimmingError: If invalid arguments are found
        """
        invalid_args = []
        for arg in args:
